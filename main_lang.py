...
client = OpenAI(api_key=OPENROUTER_API_KEY, base_url="https://openrouter.ai/api/v1")  # Инициализируем клиент OpenAI с помощью API-ключа
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 Привет! Я бот-помощник через OpenRouter Responses API.\n"
